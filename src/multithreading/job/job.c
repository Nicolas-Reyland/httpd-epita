...
void add_job_to_queue(struct job *job)
{
    ...
    if (job == NULL)
    {
        log_warn("%s: trying to push null job to queue ? push cancelled\n",
                 __func__);
        return;
    }
    job->uid = job_uid++;

    size_t q_index = job->socket_fd % g_state.max_num_threads;

    log_debug("Adding job {type = %d, socket_fd = %d, index = %zd, uid = %zu} "
              "to queue n%zu\n",
              job->type, job->socket_fd, job->index, job->uid, q_index);
    ...
    if (queue_push(g_state.job_queues[q_index], job) == -1)
        log_error("%s: failed to push job of type %d to queue\n", __func__,
                  job->type);

    log_debug("%s: queue %zu: %zu jobs left\n", __func__, q_index,
              g_state.job_queues[q_index]->size);
...
void execute_process_job(struct job *job)
{
    ...
    {
        log_error(
            "[%u] %s(lock client): could not find a client associated to %d\n",
    ...
    if (!alive)
    {
        ...
        {
            log_warn("[%u] %s: locking vector from client failed. manually "
            ...
            *job = close_job;
            ...
            log_debug("[%u] Finished (early) process job (uid = %zu)\n",
                      pthread_self(), job->uid);
            return;
        }
        else
        {
            log_debug("[%u] %s: (job uid = %zu) locked vector for client%d\n",
                      pthread_self(), __func__, job->uid, job->socket_fd);

            close_connection(client);
            int error;
            if ((error = pthread_mutex_unlock(&vhost->clients_mutex)))
                log_error("[%u] %s(manual close unlock vector, ignore):%s\n",
    ...
    log_debug("[%u] Finished process job (uid = %zu)\n", pthread_self(),
              job->uid);
...
void execute_close_job(struct job *job)
{
    ...
    if (client == NULL)
    {
        log_error("[%u] %s(lock client): could not find a client "
                  "associated to %d\n",
                  pthread_self(), __func__, job->socket_fd);
    ...
    if (lock_vector_containing_client(client) == -1)
    {
        log_error("[%u] %s(lock vector client): failed to lock vector "
                  "associated to %d\n",
                  pthread_self(), __func__, job->socket_fd);
        release_client(client);
    ...
    // unlock vector (NOT the client, since it has been removed)
    {
        ...
        if ((error = pthread_mutex_unlock(&vhost->clients_mutex)))
        {
            log_error("[%u] %s(unlock clients vector, ignored): %s\n",
                      pthread_self(), __func__, strerror(error));
        }
    }
...
int lock_vector_containing_client(struct client *client)
{
    ...
    {
        log_error("[%u] %s(client vhost check): no vhost associated to the "
                  "client %d\n",
    ...
    // lock vector in which the client resides
    {
        ...
        {
            log_error("[%u] %s(lock clients vector): %s\n", pthread_self(),
                      __func__, strerror(error));
            return -1;
        }
    }
